...
bool ShaderComponent::CompileAttach(){
	...
	try { 		
		...
		// This requires a handle. vsText is already a pointer but GL shader source signature want a character **
		// We need the address to the pointer to the charcter array
		// The reason is it's not neccessary to send the entire document
		// We could do it one line at a time, an array of lines of code, but we'll just read the whole thing (the 1)
